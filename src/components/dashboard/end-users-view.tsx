...
export function EndUsersView({ project }: { project: Project }) {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-slate-900">End Users</h1>
        <p className="text-sm text-slate-500">{total} total recipients · page {page}</p>
      </div>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="p-12 text-center">
              ...
              <div className="text-sm text-slate-500 mb-1">No end users registered yet</div>
              <div className="text-xs text-slate-400">Register users via <code className="bg-slate-100 px-1 py-0.5 rounded">POST /api/v1/users</code></div>
            </div>
          ) : (
                ...
                return (
                  <div key={u.id} className="p-4 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-slate-500 font-mono">{u.externalId}</div>
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <Badge variant="outline" className="text-[10px]"><Smartphone className="w-3 h-3 mr-1" />{u._count?.devices || 0} devices</Badge>
                      {tags.length > 0 && (
